...
export default function StoreFront() {
  ...
  const navigate = useNavigate(); // नयाँ थपिएको
  ...
  const addToCart = () => {
    // 🟢 यदि लगइन छैन भने सिधै AuthPage मा पठाउने
    if (!currentUser) {
    ...
    if (existing) {
      ...
      setCart([...cart, { 
        cartItemId, productId: selectedProduct._id, name: selectedProduct.name, image: selectedProduct.image, measureUnit: selectedTier.measureUnit, unitPrice: unitPrice, qty: Number(orderQty), finalPrice: calculatedPrice, displayUnit: displayUnitStr
      }]);
    }
  ...
  const filteredProducts = products.filter(product => {
    ...
    const matchesSearch = product.name.toLowerCase().includes(searchLower) || (product.category && product.category.toLowerCase().includes(searchLower)) || (product.description && product.description.toLowerCase().includes(searchLower));
  ...
  const sendBillAsPhoto = async () => {
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        showModal('success', 'Bill Downloaded! 📥', "हजुरको डिभाइसमा बिल सेभ भयो। अब तलको बटन थिचेर WhatsApp खोल्नुहोस् र 'Gallery' बाट भर्खरै Download भएको बिलको फोटो पठाउनुहोला।", whatsappUrl);
      }, 'image/png');
    } catch (error) {
  ...
  const handlePreviewBill = () => {
    ...
    setIsCartOpen(false); setShowInvoice(true); 
  };
    ...
    <div className="min-h-screen bg-gray-50 font-sans flex flex-col overflow-x-hidden">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col items-center p-8 relative animate-in zoom-in-95 duration-300 text-center border border-gray-100">
            ...
            {appModal.whatsappUrl ? (
              ...
              <button onClick={() => setAppModal({ ...appModal, isOpen: false })} className="w-full bg-blue-600 text-white py-4 rounded-xl font-black text-lg hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/30">Okay, Got it!</button>
            )}
          ...
          <div className="bg-gray-100 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto flex-1 flex justify-center bg-gray-200">
              <div ref={invoiceRef} className="bg-white p-8 shadow-sm w-full max-w-md mx-auto relative" style={{ fontFamily: "'Courier New', Courier, monospace" }}>
                <div className="absolute top-4 right-4 text-right text-xs text-gray-500 font-bold">Billed To:<br/><span className="text-gray-800">{currentUser?.name}</span><br/>{currentUser?.phone}</div>
                ...
                <div className="flex justify-between items-center mb-6 text-sm font-bold text-gray-700"><span>Date: {currentDate}</span><span>Invoice #SK-{Math.floor(1000 + Math.random() * 9000)}</span></div>
                <table className="w-full text-left mb-6 border-collapse">
                  <thead><tr className="border-b-2 border-black text-sm uppercase tracking-wider text-gray-800"><th className="py-2">Item</th><th className="py-2 text-right">Amount</th></tr></thead>
                  <tbody>
                    {cart.map((item, idx) => (
                      <tr key={idx} className="border-b border-gray-200 text-sm font-bold text-gray-700">
                        <td className="py-3">{item.name} <br/><span className="text-xs text-gray-500 font-normal">{item.displayUnit} (Rate: Rs {item.unitPrice.toFixed(2)}/1{item.measureUnit})</span></td>
                        <td className="py-3 text-right">Rs {item.finalPrice.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                <div className="border-t-2 border-black pt-4 mb-8 flex justify-between items-center text-xl font-black text-gray-900"><span>GRAND TOTAL</span><span>Rs {totalAmount}</span></div>
                <div className="text-center border-t-2 border-dashed border-gray-300 pt-6"><p className="text-sm font-bold text-gray-800">Thank you for your order!</p><p className="text-xs text-gray-500 mt-1">Visit Again</p></div>
              </div>
            </div>
      ...
      {selectedProduct && !showInvoice && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col md:flex-row relative animate-in zoom-in-95 duration-300">
            ...
            <div className="md:w-1/2 bg-gray-100"><img src={selectedProduct.image} alt={selectedProduct.name} className="w-full h-48 md:h-full object-cover" /></div>
            <div className="md:w-1/2 p-8 flex flex-col justify-center h-full max-h-[80vh] overflow-y-auto">
                ...
                <div className="flex flex-wrap gap-3">
                  {selectedProduct.pricing && selectedProduct.pricing.length > 0 ? (
                    selectedProduct.pricing.map((tier, idx) => (
                      <button key={idx} onClick={() => { setSelectedTier(tier); setOrderQty(tier.measureQty); }} className={`px-5 py-3 border-2 rounded-xl text-left transition-all flex flex-col items-center ${selectedTier === tier ? 'border-blue-600 bg-blue-50 text-blue-800 ring-2 ring-blue-300 ring-offset-1' : 'border-gray-200 text-gray-600 hover:border-blue-300'}`}>
                        <span className="text-xl font-black">{tier.measureUnit}</span><span className="text-xs mt-1 font-semibold opacity-70">Rate: Rs {tier.price} / {tier.measureQty}{tier.measureUnit}</span>
                      </button>
                    ))
              ...
              <div className="mb-6 bg-yellow-50 p-5 rounded-2xl border border-yellow-200 shadow-inner">
                <label className="font-bold text-gray-700 block mb-3 text-lg">कति <span className="text-blue-700 font-black">{selectedTier?.measureUnit || 'सामान'}</span> चाहियो?:</label>
                <div className="flex items-center gap-4">
                  <input type="number" step="any" min="0.1" value={orderQty} onChange={(e) => setOrderQty(e.target.value)} className="border-2 border-gray-300 focus:border-blue-600 focus:ring-4 focus:ring-blue-100 rounded-xl p-3 w-32 font-black text-2xl text-center outline-none transition bg-white" />
                  <span className="font-black text-gray-500 text-xl uppercase">{selectedTier?.measureUnit}</span>
                </div>
              </div>
              <div className="text-3xl font-black text-blue-700 mb-6 border-b pb-4">Total: Rs { ((getUnitPrice(selectedTier)) * (Number(orderQty) || 0)).toFixed(2) }</div>
              <div className="mb-6 flex-1"><p className="text-gray-700 text-sm leading-relaxed">{selectedProduct.description || "यस प्रोडक्टको बारेमा धेरै जानकारी उपलब्ध छैन।"}</p></div>
              <button onClick={addToCart} className="w-full bg-blue-600 text-white py-4 rounded-xl font-black text-lg hover:bg-blue-700 transition shadow-lg shadow-blue-500/30 flex items-center justify-center gap-2 hover:-translate-y-1 active:scale-95"><ShoppingCart size={24} /> Add to Cart</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2 text-2xl font-black tracking-tight cursor-pointer" onClick={() => window.scrollTo(0,0)}>
              <Store size={32} className="text-yellow-400" /><span className="hidden sm:block">सन्दिप किराना स्टोर</span><span className="sm:hidden text-xl">सन्दिप किराना</span>
            </div>
            <div className="hidden md:flex space-x-8 font-semibold">
              <a href="#" className="text-yellow-400 border-b-2 border-yellow-400 pb-1">Shop</a><a href="#about" className="hover:text-yellow-300 transition">About Us</a><a href="#contact" className="hover:text-yellow-300 transition">Contact</a>
            </div>
            <div className="flex items-center gap-4">
              {currentUser ? (
                <div className="hidden sm:flex items-center gap-3 bg-blue-900 px-4 py-2 rounded-full border border-blue-700 shadow-inner">
                  <User size={18} className="text-yellow-400" /><span className="font-bold text-sm truncate max-w-[120px]">{currentUser.name}</span><button onClick={handleLogout} className="text-red-300 hover:text-red-400 ml-2 transition" title="Logout"><LogOut size={18} /></button>
                </div>
              ) : (
                <button onClick={() => navigate('/login')} className="hidden sm:flex items-center gap-2 bg-yellow-400 text-blue-900 px-5 py-2 rounded-full font-black hover:bg-yellow-300 transition shadow-md hover:-translate-y-0.5"><User size={18} /> Login</button>
              )}
              <button onClick={() => setIsCartOpen(!isCartOpen)} className="relative p-2 bg-blue-700 rounded-full hover:bg-blue-600 transition shadow-inner flex items-center gap-2 px-5 group">
                <ShoppingCart size={22} className="group-hover:scale-110 transition" /><span className="font-bold hidden sm:block text-lg">Rs {totalAmount}</span>
                {totalItems > 0 && <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold w-6 h-6 flex items-center justify-center rounded-full border-2 border-blue-800 shadow-sm animate-pulse">{totalItems > 99 ? '99+' : totalItems}</span>}
              </button>
              <button className="md:hidden p-1 hover:bg-blue-700 rounded-lg transition" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>{isMobileMenuOpen ? <X size={28} /> : <Menu size={28} />}</button>
            </div>
          </div>
        </div>
        <div className={`md:hidden absolute w-full left-0 bg-blue-900 border-b border-blue-700 shadow-2xl transition-all duration-300 ease-in-out overflow-hidden ${isMobileMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
          <div className="px-4 py-4 space-y-4">
            {currentUser ? (
               ...
               <button onClick={() => { setIsMobileMenuOpen(false); navigate('/login'); }} className="w-full bg-yellow-400 text-blue-900 p-3 rounded-xl font-black flex justify-center items-center gap-2 mb-4 shadow-md"><User size={20}/> Login / Register</button>
            )}
      ...
      <main className="max-w-7xl mx-auto p-4 py-6 flex-1 w-full flex flex-col lg:flex-row gap-8 relative">
        <div className="flex-1 w-full">
          <div className="bg-gradient-to-br from-blue-700 via-blue-600 to-indigo-800 rounded-3xl p-8 mb-8 shadow-2xl flex flex-col md:flex-row items-center justify-between overflow-hidden relative">
            ...
            <div className="md:w-1/3 mt-8 md:mt-0 relative z-10 hidden md:block"><img src="https://scontent.fktm17-1.fna.fbcdn.net/v/t39.30808-6/480559092_594232123435745_383012169672590612_n.jpg?stp=dst-jpg_tt6&cstp=mx1158x2048&ctp=s1158x2048&_nc_cat=108&ccb=1-7&_nc_sid=a5f93a&_nc_ohc=R_d_qVcN2K8Q7kNvwH0zNRP&_nc_oc=Adqj1Cmz1eQ_tArCZd-eU5EMiYHd7wKEAQDvcvlmDVm2vWPiO_SzxN3NGfK0JW8q5-1Vu1ofZYyUiDT60rEj0iy8&_nc_zt=23&_nc_ht=scontent.fktm17-1.fna&_nc_gid=kAX7SIV8Dz7HGCACXbjiVA&_nc_ss=7b2a8&oh=00_Af9h2bzTnc-M-eWwJfnHMtz9hWLcTCpLnVryGWOKQOmozQ&oe=6A3899EB" alt="Groceries" className="rounded-2xl shadow-2xl transform rotate-3 hover:rotate-0 transition duration-500 hover:scale-105" /></div>
              ...
              filteredProducts.map(product => {
                ...
                return (
                  <div key={product._id} className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 flex flex-col group cursor-pointer" onClick={() => setSelectedProduct(product)}>
                    <div className="relative overflow-hidden bg-gray-100"><img src={product.image} alt={product.name} className="w-full h-40 md:h-48 object-cover group-hover:scale-110 transition-transform duration-500" /><div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center"><span className="bg-white text-gray-900 font-bold py-2 px-4 rounded-full text-sm">View Details</span></div></div>
                    <div className="p-4 md:p-5 flex flex-col flex-1">
                      <div className="flex justify-between items-start mb-2"><span className="text-[10px] text-blue-600 font-bold tracking-widest uppercase bg-blue-50 px-2 py-1 rounded">{product.category}</span><span className="text-[10px] text-gray-500 font-bold bg-gray-100 px-2 py-1 rounded truncate max-w-[80px]">per {defaultTier.measureUnit}</span></div>
                      <h3 className="text-sm md:text-base font-bold text-gray-800 flex-1 leading-snug line-clamp-2">{product.name}</h3>
                      <div className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between"><div className="text-lg md:text-xl font-black text-gray-900">Rs {defaultTier.price}</div><button onClick={(e) => { e.stopPropagation(); setSelectedProduct(product); }} className="bg-blue-600 text-white p-2 md:p-2.5 rounded-xl hover:bg-blue-700 hover:shadow-lg transition-all active:scale-95"><Plus size={20} /></button></div>
                    </div>
                  </div>
                );
              })
            ) : <div className="col-span-full text-center py-20 bg-white rounded-2xl border border-dashed border-gray-300"><Search size={64} className="mx-auto mb-4 text-gray-300 animate-bounce" /><p className="text-xl font-bold text-gray-500">तपाईंले खोज्नुभएको सामान भेटिएन।</p></div>}
          </div>
        </div>
        ...
        {isCartOpen && (
          <div className="lg:w-[400px] w-full bg-white p-6 rounded-t-3xl lg:rounded-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.1)] lg:shadow-2xl border border-gray-100 h-fit lg:sticky lg:top-24 z-30 fixed bottom-0 left-0 max-h-[85vh] overflow-y-auto lg:max-h-[calc(100vh-120px)] flex flex-col animate-in slide-in-from-bottom lg:slide-in-from-right duration-300">
            <div className="flex justify-between items-center mb-6 sticky top-0 bg-white z-10 pb-4 border-b"><h2 className="text-xl md:text-2xl font-black flex items-center gap-3 text-gray-800"><ShoppingCart className="text-blue-600" size={28} /> Your Cart</h2><button onClick={() => setIsCartOpen(false)} className="lg:hidden bg-gray-100 p-2 rounded-full text-gray-500 hover:text-red-500 hover:bg-red-50 transition"><X size={20} /></button></div>
            {cart.length === 0 ? (
              <div className="text-center py-12 flex-1 flex flex-col items-center justify-center"><img src="https://cdn-icons-png.flaticon.com/512/11329/11329060.png" alt="Empty Cart" className="w-32 mb-6 opacity-40 grayscale hover:grayscale-0 transition duration-500"/><p className="text-gray-500 font-medium text-lg">Cart खाली छ!</p></div>
            ) : (
              <div className="space-y-4 flex-1">
                {cart.map(item => (
                  <div key={item.cartItemId} className="flex justify-between items-center bg-white p-3 md:p-4 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition relative">
                    ...
                    <div className="flex-1 pr-2"><h4 className="font-bold text-sm text-gray-800 line-clamp-1">{item.name}</h4><p className="text-[10px] text-blue-600 font-black bg-blue-50 w-max px-2 py-0.5 rounded mt-1">{item.displayUnit}</p><p className="text-gray-900 text-sm font-black mt-1">Rs {item.finalPrice.toFixed(2)}</p></div>
                    <div className="flex items-center gap-2"><input type="number" step="any" value={item.qty} onChange={(e) => handleCartQtyChange(item.cartItemId, e.target.value)} className="w-16 p-1.5 text-center font-bold border-2 border-gray-200 rounded-lg outline-none focus:border-blue-500 text-sm" /></div>
            ...
            {cart.length > 0 && (
              <div className="pt-6 mt-6 border-t-2 border-dashed border-gray-200 bg-white sticky bottom-0 z-10 pb-2">
                <div className="flex justify-between items-end mb-6"><span className="text-gray-500 font-bold uppercase tracking-wider text-sm">Total Amount</span><span className="text-3xl font-black text-blue-700">Rs {totalAmount}</span></div>
                <button onClick={handlePreviewBill} className="w-full bg-green-500 text-white py-4 rounded-xl font-black text-lg hover:bg-green-600 transition-all shadow-lg shadow-green-500/30 flex justify-center items-center gap-3 hover:-translate-y-1 active:scale-95">Generate Invoice & Order</button>
              </div>
            )}
          </div>
        )}
      ...
      <footer id="contact" className="bg-gray-900 text-gray-300 py-12 mt-12">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-8">
          <div><div className="flex items-center gap-2 text-2xl font-black text-white mb-4"><Store className="text-yellow-400" /> सन्दिप किराना स्टोर</div><p className="text-gray-400 leading-relaxed">Hajur ko dainik upabhogya saman ko viswasilo sathi.</p></div>
          <div><h3 className="text-white font-bold text-lg mb-4">Contact Info</h3><ul className="space-y-3"><li className="flex items-center gap-3"><Phone size={18} className="text-blue-400"/> {WHATSAPP_NUMBER}</li><li className="flex items-center gap-3"><MapPin size={18} className="text-blue-400"/> सुर्यबिनायक्,१, भक्तपुर </li></ul></div>
          <div><h3 className="text-white font-bold text-lg mb-4">Quick Links</h3><ul className="space-y-2"><li><a href="#" className="hover:text-yellow-400 transition">Home</a></li><li><a href="#" className="hover:text-yellow-400 transition">Shop Products</a></li></ul></div>
        </div>
      </footer>
    </div>
  );
